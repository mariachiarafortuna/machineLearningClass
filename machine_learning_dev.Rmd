...
l1 <- glm(data = dc_l, ALIGN ~ SEX, family = "binomial")

summary(l1)

post_l1 <- predict(l1, type = "response")

dc_l$ALIGN_pred <- ifelse(post_l1>0.5,  "Good Characters", "Bad  Characters") 

misClasificError <- mean(ALIGN_pred != dc_l$ALIGN)
print(paste('Accuracy',1-misClasificError))

table(dc_l$ALIGN)
table(ALIGN_pred)

table(dc_l$ALIGN, ALIGN_pred)
...
ggp <- ggplot(data = dc_l, mapping = aes(x = active_years, y = ALIGN)) +
  geom_point(colour="blue") +
  #geom_line(mapping = aes(x = active_years, y = ALIGN), colour="red") +
  facet_wrap(facets = ~SEX)
print(ggp)

l2 <- glm(data = dc_l, ALIGN ~ SEX + active_years, family = "binomial")
...
misClasificError <- mean(ALIGN_pred_l2 != dc_l$ALIGN)
print(paste('Accuracy',1-misClasificError))

table(dc_l$ALIGN, ALIGN_pred_l2)
